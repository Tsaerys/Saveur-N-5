...
// ── EXPORT PDF (audit #23) ────────────────────────────────────────────────
// jsPDF UMD est chargé depuis CDN. Si absent (hors-ligne), on tombe sur window.print().
function exportRecipePDF(){
  var r = S.recipe;
  if(!r){ toast('Aucune recette ouverte',2000,'error'); return; }

  // Vérifier que jsPDF est disponible
  var jsPDFCtor = (window.jspdf && window.jspdf.jsPDF) || (typeof jsPDF !== 'undefined' && jsPDF);
  if(!jsPDFCtor){
    toast('PDF non disponible hors-ligne — impression à la place',3000);
    window.print();
    return;
  }

  var doc = new jsPDFCtor({ orientation:'portrait', unit:'mm', format:'a4' });
  var PW  = 210;   // largeur A4
  var PH  = 297;   // hauteur A4
  var ML  = 15;    // marge gauche
  var MR  = 15;    // marge droite
  var CW  = PW - ML - MR;  // largeur utile
  var y   = 0;             // curseur vertical

  // ─── Couleurs ───
  var GOLD  = [154, 111, 42];
  var DARK  = [44,  34,  24];
  var GREY  = [100, 90,  75];
  var LIGHT = [230, 224, 212];
  ...
  function textWrap(txt, x, startY, maxW, lineH){
  ...
  function checkPage(needed){
    if(y + needed > PH - 18){
      doc.addPage();
      y = 18;
      // Footer sur page précédente déjà rendu — nouveau header léger
      doc.setFont('helvetica','italic');
      doc.setFontSize(7);
      setColor(GREY);
      doc.text('Saveur No 5  —  ' + r.nom, ML, 10);
      setDraw(LIGHT);
  ...
  // ─── BANDE HEADER ───
  var co = COUNTRY_COLORS && COUNTRY_COLORS[r.co] ? COUNTRY_COLORS[r.co] : '#9a6f2a';
  // Convertir hex → rgb
  var coRgb = (function(h){
    h = h.replace('#','');
    if(h.length === 3) h = h[0]+h[0]+h[1]+h[1]+h[2]+h[2];
    return [parseInt(h.slice(0,2),16), parseInt(h.slice(2,4),16), parseInt(h.slice(4,6),16)];
  })(co);
  doc.setFillColor(coRgb[0], coRgb[1], coRgb[2]);
  ...
  doc.setFontSize(22);
  setColor(WHITE);
  var nomLines = doc.splitTextToSize(r.nom, CW);
  var nomY = nomLines.length > 1 ? 16 : 22;
  doc.text(nomLines, ML, nomY);

  // Chef + origine
  doc.setFont('helvetica', 'italic');
  doc.setFontSize(9);
  setColor([220, 200, 160]);
  var sousTitre = r.chef + '  ·  ' + r.co;
  doc.text(sousTitre, ML, nomY + nomLines.length * 9 + 2);

  y = 50;

  // ─── BANDEAU META ───
  var metaItems = [
    { ico: 'Portions', val: String(S.portions) },
    { ico: 'Prep', val: r.prep + ' min' },
    { ico: 'Cuisson', val: r.cui + ' min' },
    { ico: 'Total', val: (r.prep + r.cui) + ' min' },
    { ico: 'Difficulte', val: ['','Facile','Moyen','Difficile','Expert','Maitre'][r.diff] || String(r.diff) }
  ];
  var metaW = CW / metaItems.length;
  setDraw(LIGHT);
  doc.setLineWidth(0.3);
  ...
  metaItems.forEach(function(m, i){
    ...
    doc.setFontSize(7);
    setColor(GREY);
    doc.text(m.ico.toUpperCase(), mx, y + 3);
    doc.setFont('helvetica','bold');
    doc.setFontSize(10);
    setColor(DARK);
    doc.text(m.val, mx, y + 9);
    if(i > 0){ doc.setDrawColor(220,215,205); doc.setLineWidth(0.25); doc.line(mx - 1, y - 1, mx - 1, y + 11); }
  });
  doc.line(ML, y + 12, PW - MR, y + 12);
  y += 18;

  // ─── COLONNES ───
  var colW  = CW * 0.38;   // colonne ingrédients
  var colR  = ML + colW + 6; // début colonne étapes
  var colRW = CW - colW - 6; // largeur colonne étapes
  ...
  doc.setFontSize(9);
  setColor(GOLD);
  doc.text('INGREDIENTS', ML, iy);
  setDraw(GOLD);
  doc.setLineWidth(0.5);
  doc.line(ML, iy + 1.5, ML + colW, iy + 1.5);
  iy += 6;

  // Portions si modifiées
  if(S.portions !== r.bp){
    doc.setFont('helvetica','italic');
    doc.setFontSize(7.5);
    setColor(GREY);
    doc.text('(pour ' + S.portions + ' pers.)', ML, iy);
    iy += 4.5;
  }

  r.ig.forEach(function(ig){
    var nom = ig[0]; var qty = ig[1]; var unit = ig[2];
    var qtyStr = unit === 'qs' ? 'q.s.' : fmtQty(qty * ratio, unit);
    doc.setFont('helvetica','bold');
    doc.setFontSize(8);
    setColor(GOLD);
    doc.text(qtyStr, ML, iy);
    doc.setFont('helvetica','normal');
    setColor(DARK);
    var nomLines2 = doc.splitTextToSize(nom, colW - 18);
    doc.text(nomLines2, ML + 18, iy);
    iy += nomLines2.length * 4.2 + 0.8;
    if(iy > PH - 18){ iy = y + 6; colW = colW; } // débordement très rare
  });
  ...
  if(r.vin){
    ...
    doc.setFontSize(8);
    setColor(DARK);
    var vinLines = doc.splitTextToSize(r.vin, colW);
    doc.text(vinLines, ML, iy);
    iy += vinLines.length * 4.2;
  }

  // ── Ligne séparatrice verticale ──
  var endColY = Math.max(iy, y + 40);
  setDraw(LIGHT);
  doc.setLineWidth(0.4);
  doc.line(ML + colW + 3, y - 2, ML + colW + 3, endColY + 10);
  ...
  doc.setFontSize(9);
  setColor(GOLD);
  doc.text('PREPARATION', colR, sy);
  setDraw(GOLD);
  doc.setLineWidth(0.5);
  doc.line(colR, sy + 1.5, colR + colRW, sy + 1.5);
  sy += 6;

  var steps = r.et.split('\n').filter(function(s){ return s.trim(); });
  var stepNum = 0;
  steps.forEach(function(s){
    var isHead = !s.match(/^\d+[\.)]/);  // ligne-titre de section
    if(sy > PH - 22){ doc.addPage(); sy = 18; }
    if(isHead){
      doc.setFont('helvetica','bolditalic');
      doc.setFontSize(8);
      setColor(GREY);
      var hl = doc.splitTextToSize(s.trim(), colRW);
      doc.text(hl, colR, sy);
      sy += hl.length * 4.5 + 2;
      return;
    }
    stepNum++;
    var txt = s.replace(/^\d+[\.)\s]+/, '');
    // Numéro cerclé (carré)
    doc.setFillColor(GOLD[0], GOLD[1], GOLD[2]);
    doc.roundedRect(colR, sy - 3.5, 5, 5, 1, 1, 'F');
    doc.setFont('helvetica','bold');
    doc.setFontSize(7);
    setColor(WHITE);
    doc.text(String(stepNum), colR + (stepNum < 10 ? 1.5 : 0.8), sy + 0.2);
    // Texte de l'étape
    doc.setFont('helvetica','normal');
    doc.setFontSize(8.5);
    setColor(DARK);
    var tl = doc.splitTextToSize(txt, colRW - 7);
    doc.text(tl, colR + 7, sy);
    sy += tl.length * 4.5 + 2;
  });

  // ── Notes (si présentes) ──
  if(r.notes){
    var ny = Math.max(sy, iy) + 8;
    if(ny > PH - 30){ doc.addPage(); ny = 18; }
    doc.setLineWidth(0.3);
    setDraw(GOLD);
    doc.setFillColor(253, 246, 232);
    var noteLines = doc.splitTextToSize('Notes : ' + r.notes, CW - 6);
    var noteH = noteLines.length * 4.2 + 6;
    doc.roundedRect(ML, ny, CW, noteH, 2, 2, 'FD');
    doc.setFont('helvetica','italic');
    doc.setFontSize(8);
    setColor(DARK);
    doc.text(noteLines, ML + 3, ny + 4.5);
    y = ny + noteH + 4;
  }
  ...
  for(var p = 1; p <= totalPages; p++){
    ...
    doc.setFontSize(7);
    setColor(GREY);
    doc.text('Saveur No 5  —  ' + r.nom, ML, PH - 8);
    doc.text('Page ' + p + ' / ' + totalPages, PW - MR, PH - 8, { align:'right' });
  }

  // ── Téléchargement ──
  var safeNom = r.nom.replace(/[^a-zA-Z0-9\u00C0-\u024F\s-]/g, '').trim().replace(/\s+/g,'-').toLowerCase();
  doc.save('saveur-n5-' + safeNom + '.pdf');
  toast('📄 PDF généré : ' + r.nom, 3000, 'success');
}
