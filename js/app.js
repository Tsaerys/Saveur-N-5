// Saveur N°5 — Application


// ── CONSTANTS ─────────────────────────────────────────────────────────────
const FLAGS={France:"🇫🇷",Italie:"🇮🇹","Grèce":"🇬🇷",Espagne:"🇪🇸",Asie:"🌏","États-Unis":"🇺🇸",Mexique:"🇲🇽",Maroc:"🇲🇦",Liban:"🇱🇧",Portugal:"🇵🇹",Scandinavie:"🇸🇪",Allemagne:"🇩🇪","Royaume-Uni":"🇬🇧",Argentine:"🇦🇷","Moyen-Orient":"🌍",Tunisie:"🇹🇳","Éthiopie":"🇪🇹",Pérou:"🇵🇪",Brésil:"🇧🇷","Amérique du Sud":"🌎",Cuba:"🇨🇺","Caraïbes":"🌴",Pologne:"🇵🇱",Hongrie:"🇭🇺","Europe Centrale":"🇪🇺",Afrique:"🌍","Sénégal":"🇸🇳",Canada:"🇨🇦",Turquie:"🇹🇷"};const COUNTRIES=[...new Set(RECIPES.map(r=>r.co))];
...
// ── STATE ─────────────────────────────────────────────────────────────────
var _wakeLock=null;
let S={view:"browse",recipe:null,portions:4,filters:{co:"",cat:"",diff:"",time:"",q:"",regime:""},frigo_active:false,frigo_ings:[],frigo_strict:false,variant:null,cooking:null,cooking_step:0,timer_interval:null,timer_remaining:0,timer_running:false,unit_mode:"metric",menu_generated:null};
...
function addRecent(id){RECENT=RECENT.filter(i=>i!==id);RECENT.unshift(id);if(RECENT.length>8)RECENT=RECENT.slice(0,8);saveRecent();}
...
function dots(d){let s="";for(let i=1;i<=5;i++)s+=`<span class="dot${i<=d?" on":""}"></span>`;return s;}
function starHtml(id,size=14,interactive=false){
  const r=RATINGS[id]||0;
  if(!interactive)return r===0?"":`<span style="color:#e8a020;font-size:${size}px">${"★".repeat(r)}</span>`;
  return[1,2,3,4,5].map(i=>`<span class="rs${i<=r?" on":""}" onclick="rateRecipe('${id}',${i})">★</span>`).join("");
}
...
function fmtTimerFT(sec){if(sec<=0)return"0:00";const m=Math.floor(sec/60);const s=sec%60;return`${m}:${s.toString().padStart(2,"0")}`;}

// ── FILTERED LIST ─────────────────────────────────────────────────────────
function filtered(){
  const {co,cat,diff,time,q,regime}=S.filters;
  ...
  if(S.frigo_active&&S.frigo_ings.length){
    ...
    recs=recs.filter(r=>r.nom.toLowerCase().includes(sq)||r.chef.toLowerCase().includes(sq)||r.ig.some(([n])=>n.toLowerCase().includes(sq))||r.sous.toLowerCase().includes(sq));
  }
  return recs;
}
...
function renderFilters(){
  const {co,cat,diff,time,regime}=S.filters;
...
function renderMain(){
  ...
  const cards=recs.map(r=>{
    ...
    return`<div class="card" data-id="${r.id}">
      ...
      ${ratingHtml}
      <div class="card-body">
        <div class="card-top"><span class="cat-badge cat-${catClass(r.cat)}">${r.cat}</span>${(totTime(r)<30)?'<span class="badge-express">⏱ Express</span>':''}${isVege(r)?'<span class="badge-vege">🌿 Végé</span>':''}
      </div>
    </div>`;
  }).join("");
  main.innerHTML=`<div class="main-content"><div class="grid">${cards}</div></div>`;
...
// manifest.json is a static file — see /manifest.json
...
// -- ALERTE SONORE + VIBRATION MINUTEUR --
...
function openRandom(){
  var pool=S.filters.co||S.filters.cat?filtered():RECIPES;
...
initUserRecipes();updateBadges();renderFilters();renderRecent();initTheme();initScrollTop();
...
function shareCoursesText(){
  ...
  if(navigator.share){navigator.share({title:'Ma liste de courses — Saveur N°5',text:txt}).catch(function(){});}
  else{_copyToClipboard(txt,'📋 Liste de courses copiée !');}
}
